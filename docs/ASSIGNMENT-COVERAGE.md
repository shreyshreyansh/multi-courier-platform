...
This document is the reviewer-facing scope statement for the backend assignment. It distinguishes verified behavior from the production slices that are still required. It is intentionally evidence-led: a capability is marked implemented only when there is a direct route/source link and automated proof; a production guarantee is not inferred from an in-memory demonstration.
...
3. **Reliable dispatch** - replace the in-process scheduler with transactional outbox records, a BullMQ queue, worker, idempotent job keys, retry policy, and safe auth refresh.
4. **Durable batch state** - persist batch/item records atomically with admission and retain per-order completion/read state across restarts.
